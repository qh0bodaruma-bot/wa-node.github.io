...
// Replace 1: Edo Travelogue Series
const target1 = `        <div class="book-showcase">
          {/* Item 1 */}
...
// Normalize content strings to prevent matching failures due to whitespace differences
const normalize = (str) => str.replace(/\s+/g, ' ').trim();

const normalizedContent = normalize(content);
const normalizedTarget1 = normalize(target1);

if (normalizedContent.includes(normalizedTarget1)) {
  console.log('Target 1 matched!');
  // Let's do exact match replacement or locate using substrings
  // Since white space might slightly differ, we will replace based on standard structure
  
  // We can do standard replace since we know the content
  const startIndex = content.indexOf('        <div class="book-showcase">');
  const endIndex = content.indexOf('          {/* Item 9: Travels in Edo (江戸の旅) */}');
  
  // Actually, let's locate the exact showcase and replace
  // We will find the range: from <div class="book-showcase"> to </div>\n      </div>\n    </section>
  ...
  if (section1Start !== -1) {
    ...
    const showcaseEnd = content.indexOf('</div>\n      </div>\n    </section>', showcaseStart);
    if (showcaseStart !== -1 && showcaseEnd !== -1) {
      const targetSegment = content.substring(showcaseStart, showcaseEnd + 6); // include the "</div>"
      content = content.replace(targetSegment, replacement1);
      console.log('Successfully replaced Section 1 (Edo Travelogue)');
    }
  }
} else {
  console.warn('Target 1 match failed, replacing using index markers');
  const section1Start = content.indexOf('<section class="kdp-section premium-dark">');
  if (section1Start !== -1) {
    const showcaseStart = content.indexOf('<div class="book-showcase">', section1Start);
    const nextSection = content.indexOf('<!-- Section 2:', showcaseStart);
    if (showcaseStart !== -1 && nextSection !== -1) {
      // Find the last </div> before next section
      ...
      if (showcaseEnd !== -1 && showcaseEnd > showcaseStart) {
        ...
        content = content.replace(targetSegment, replacement1);
        console.log('Replaced Section 1 using markers');
      }
    }
  }
}

// Replace 2: Kawaii and Choju-giga
const target2 = `    <!-- Section 2: かわいい浮世絵シリーズ (Modern pop) -->`;
// We will locate Section 2 and replace everything up to Section 3's end
...
`;
  ...
  console.log('Successfully replaced Section 2 and 3');
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('KDP books update script completed successfully.');
